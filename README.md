...
The package employs a specific mean centering and sd scaling type, where means and sds are calculated without outliers. Used as an alternarive to meadian and mad, sinse in sparse datasets they often end up as 0.
...
For the next step we expect you to store your clonotype information in object@metadata$clone_id. We will perform an extra denoising step on every expanded clone.
...
This is done in case the same barcode corresponds to different pMHC within the same experiment, but in different donors, that were demultiplexed by hashing antibodies. In this simple example, rownames in the
