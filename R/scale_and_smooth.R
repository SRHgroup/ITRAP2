...
#' Perform pMHC Assignment for Each Big Clone
#'
...
assign_pmhc <- function(object, slot='scale.data', assay='pMHC', 
         ...
         n_tests=10, p_alpha=0.1, predefined_extreme_params=NULL, reestimate_extreme=FALSE,
  ...
  for (i in 1:n_iter) {
    ...
    if (!is.na(gap_pos)) {
      ...
      if (assignment == 'delta_gap'){
        ...
        rosner <- tryCatch({
          rosnerTest(scores, k = kmax, alpha = rosner_alpha, 
        ...
        extr <- extRemes::fevd(scores, #' Centers and scales a rows in numeric matrix
#'
#' Centers and scales a rows in numeric matrix, you have an option to replace 0 sd
#' with 0.001 not to cause a division by zero during the scaling
#'
  ...
  for (i in 1:n_iter) {
    ...
    if (!is.na(gap_pos)) {
      ...
      if (assignment == 'delta_gap'){
        ...
        if (extreme_type == 'reestimate_params'){
