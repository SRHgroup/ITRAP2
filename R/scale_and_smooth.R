...
smooth_pmhc <- function(object, best_params = NULL, assay = 'pMHC', cap_upper_quantiles=T,
                        cl_size_thresh = 3, normalise = FALSE, span_val = 1, 
                        degree_val = 1, family_val = "symmetric", verbose = TRUE) {
      ...
      error_occurred <- tryCatch({
        ...
        t <- try({
          if (cap_upper_quantiles){
            counts <- cap_high_outliers(counts, )
          }
...
assign_pmhc <- function(object, slot='scale.data', assay='pMHC', 
         assignment='rosner', params = 'remove_topx', remove_for_params = 3, assign_small_clones=F, 
         cl_size_thresh=3, entropy_thresh=1, kmax=10, rosner_alpha=0.01, extreme_alpha=0.01, delta_threshold = 1, 
         n_tests=10, p_alpha=0.1, predefined_extreme_params=NULL, extreme_type = 'regular',
    ...
    if (!is.na(gap_pos)) {
      ...
      if (assignment == 'delta_gap'){
        ...
        rosner <- tryCatch({
          library(shades)
          library(EnvStats)
          rosnerTest2(scores, k = kmax, alpha = rosner_alpha, 
        ...
        extr <- extRemes::fevd(x = scores, type = 'Gumbel')
        #extr <- extRemes::fevd(x = scores)
        
        location <- extr$results$par["location"]
        scale <- extr$results$par["scale"]
        #shape <- extr$results$par["shape"]
        
        if (extreme_type == 'reestimate_params'){ # might be an overkill
          pvalues_i <- 1-extRemes::pevd(q = scores, loc = location, scale = scale, type = 'Gumbel')
          ...
          while(pvalues_i[length(pvalues_i)] < extreme_alpha){
            ...
            scores_iter <- scores_iter[-length(scores_iter)]
            
            if (all(scores_iter == 0)){
              break
            }
            
            extr_i <- extRemes::fevd(scores_iter, type = 'Gumbel')
            
            location_i <- extr_i$results$par["location"]
            scale_i <- extr_i$results$par["scale"]
            #shape_i <- extr_i$results$par["shape"]
            
            pvalues_i <- 1-extRemes::pevd(q = scores_iter, loc = location_i, scale = scale_i, type = 'Gumbel')
            
            if (iter >= k){
              break
            }
          }
          pvalues <- 1-extRemes::pevd(q = scores, loc = location_i, scale = scale_i, type = 'Gumbel')
        }  else if (extreme_type == 'extreme_without_tail'){
          
          scores_new <- scores[1:(length(scores)-3)]
          
          extr_i <- extRemes::fevd(scores_new, type = 'Gumbel')
          ...
          pvalues <- 1-extRemes::pevd(q = scores, loc = location_i, scale = scale_i, type = 'Gumbel')
        } else if (extreme_type == 'regular') {
          pvalues <- 1-extRemes::pevd(q = scores, loc = location, scale = scale, type = 'Gumbel')
        ...
        outliers <- scores[which(pvalues<extreme_alpha)]
